...
class Parser(object): 
    ...
    def parse_sentence(self, words, pos):
        ...
        while state.buffer: 
            ...
            probs=self.model.predict(features.reshape(1,-1)).flatten()
            sorted_actions=np.argsort(probs)[::-1]
            
            for i in sorted_actions:
                action,dep_relation=self.output_labels[i]
                if action=='shift':
                    if len(state.buffer)==1 and len(state.stack)==0:
                        ...
                        state.shift()
                        break
                elif action=='left_arc' and len(state.stack)>0 and dep_relation != 'root':
