...
from conll_reader import DependencyStructure
...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        len_b=len(state.buffer)
        len_s=len(state.stack)
        nan=self.word_vocab.get('<NULL>')
        result=np.full(6,nan)
        if len_s>0:
            for i in range(1,1+min(len_s,3)):
                pos_=pos[state.stack[-i]]
                ...
                if pos is not None:
                    ...
                    pos_='<NULL>'
                if pos_ in ['<CD>','<NNP>','<UNK>','<ROOT>','<NULL>']:
                    result[i-1]=self.word_vocab.get(pos_)
                else:
                    if self.word_vocab.get(word) is not None:
                        result[i-1]=self.word_vocab.get(word)
                    else:
                        result[i-1]=self.pos_vocab.get(pos_)
        if len_b>0:
            for i in range(1,1+min(len_b,3)):
                word=words[state.buffer[-i]]
                pos_=pos[state.buffer[-i]]
                ...
                if pos_ in ['<CD>','<NNP>','<UNK>','<ROOT>','<NULL>']:
                    result[2+i]=self.word_vocab.get(pos_)
                else:
                    if self.word_vocab.get(word) is not None:
                        result[2+i]=self.word_vocab.get(word)
                    else:
                        result[2+i]=self.pos_vocab.get(pos_)
        #result=np.nan_to_num(result, nan=self.word_vocab.get('<NULL>'))
        return result


    def get_output_representation(self, output_pair):  
        a=list(zip(['left_arc']*45,dep_relations))
        b=list(zip(['right_arc']*45,dep_relations))
        c=[('shift',None)]
        a.extend(b)
        a.extend(c)
        ls=[i for i in range(0,91)]
        oh=keras.utils.to_categorical(ls, num_classes=91)
        ind=a.index(output_pair)
        return oh[ind]
